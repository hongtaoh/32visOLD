...
manual_title_doi_dict = {
	...
	'Touch and Beyond: Comparing Physical and Virtual Reality Visualizations': '10.1109/TVCG.2020.3023336',
...
def get_dois_2021_df(title_doi_dict_list, manual_title_doi_dict):
	"""combine the correct results with manual results
	"""
	title_doi_df = pd.DataFrame(title_doi_dict_list)
	cr_failed_titles_raw = list(manual_title_doi_dict.keys())
	all_titles = title_doi_df.title.tolist()
	# since titles in cr_failed_titles_raw might contain short papers. i need to exclude them
	manual_titles = [x for x in cr_failed_titles_raw if x in all_titles]
	all_dois = title_doi_df['crossref doi'].tolist()
	all_dict = dict(zip(all_titles, all_dois))
	cr_success_titles = [x for x in all_titles if x not in cr_failed_titles_raw]
	cr_success_dois = [all_dict[x] for x in cr_success_titles]
	titles_matched = cr_success_titles + manual_titles
	dois_matched = cr_success_dois + [manual_title_doi_dict[x] for x in manual_titles]
	if len(dois_matched) == 170:
		print('dois_matched has 170 dois. everything correct.')
	else:
		print('something is wrong with the total number of dois')
	df = pd.DataFrame ({
		'Conference': "VIS",
		'Year': 2021,
		'Title': titles_matched,
		'DOI': dois_matched,
	})
	return df 
...
def main(titles):
	...
	dois_2021_df = get_dois_2021_df(title_doi_dict_list, manual_title_doi_dict)
	return dois_2021_df
